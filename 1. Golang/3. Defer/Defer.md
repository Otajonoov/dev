...
Оператор `defer` позволяет запланировать вызов функции, который выполнится **непосредственно перед возвратом** из текущей функции:
...
Оператор `defer` полезен для 
- **очистки** (cleanup) — закрытие файлов, 
- освобождение ресурсов. 
Эти cleanup-действия выполнятся **независимо от того**, как функция завершится. 
Без `defer` пришлось бы выполнять cleanup-код перед каждым `return`

---
### LIFO-порядок

Под капотом Go помещает каждый `defer`-вызов в **stack**. 
При возврате из функции deferred-функции вызываются в **обратном порядке** — «последним пришёл, первым вышел» (LIFO):
...
---
### Раннее вычисление аргументов

Deferred-функция выполняется позже, но её аргументы **вычисляются сразу**, в момент написания `defer`. 
...
Output **всегда** будет «Failure», независимо от того, что вы установите позже. 
Значение `result` **захватывается** в момент `defer`.

Это также происходит с **method receiver**, что добавляет ещё один уровень путаницы:
...
Это работает так же, потому что receiver — фактически просто **аргумент**, который копируется в момент `defer`. 
Проблему можно исправить, используя **pointer receiver**:
...
---
### Два способа увидеть актуальное значение

Оба способа требуют доступа к функции, которую вы хотите отложить:
...
func main() {
    ...
    // Способ 1: Использовать pointer
    defer print(&x)
    x = 20

    // Способ 2: Использовать closure
    defer func() {
        print(&x)
    }()
    x = 30
...
Второе решение использует **closure**, которое захватывает `x` по ссылке. 
В результате при изменении `x` closure увидит новое значение — поэтому выводится 30.

---
### Defer привязан к функции, не к блоку

Оператор `defer` **всегда привязан** к окружающей функции, а не к меньшему scope. 
Частая ошибка — размещение `defer` внутри цикла с ожиданием, что он выполнится в конце каждой итерации. 
На самом деле все deferred-вызовы выполнятся **только при завершении функции**:
...
Все deferred-вызовы в цикле и блоке выполняются **в конце** `main`, в обратном порядке.

Это важно при открытии множества файлов в цикле с `defer` для их закрытия. 
Если использовать `defer` в цикле — все файлы останутся **открытыми** до конца функции. Лучший способ — использовать **wrapper-функцию** или анонимную функцию:

![](../../assets/obsidian-images/Pasted%20image%2020260219111054.png)|![](../../assets/obsidian-images/Pasted%20image%20.png)

Первый пример ждёт закрытия **всех файлов** до конца функции, что может потребить много ресурсов. 
Второй пример закрывает каждый файл **сразу** после завершения работы с ним.

---
### Изменение return-значения через defer

Если `defer` выполняется прямо перед возвратом — может ли он **изменить** return-значение? 
Ответ — **абсолютно да**, если использовать named return value:
...
// Output: After doSomething 100
```

Это работает, потому что deferred-функция может **обновить** named return value до фактического возврата.

---

## Как Defer реализован

Может показаться, что компилятор Go просто перемещает deferred-вызов в конец функции, перед каждым `return`. Для простейших случаев эта ментальная модель работает:
...
Однако **это не то, как defer работает** на самом деле, особенно в сложных случаях. Go требует **дополнительной runtime-поддержки**.
...
Здесь `defer fmt.Println("b1")` выполняется **только если** `b1` равно `true` в runtime. Компилятор не может знать это заранее.

Кроме того, аргументы deferred-функции **всегда вычисляются сразу** при достижении `defer`. Они не откладываются до момента выполнения.
...
_Figure 113. The Defer Record (src/runtime/runtime2.go)_
...
> **Иллюстрация 192.** Deferred-вызовы управляются как linked list _(Рисунок не загружен)_
...
_Figure 114. Goroutine Representation (src/runtime/runtime2.go)_

Может показаться странным, что deferred-функции группируются по **goroutine**, а не по функции. Однако `return` из функции — не единственный способ запуска defer. При panic **все** defer в текущем goroutine stack также выполняются.

Эта структура образует **singly linked list** записей `_defer` для всего call stack. Поле `_defer` goroutine — **head** этого списка:

> **Иллюстрация 193.** Все активные defer связаны в текущей goroutine _(Рисунок не загружен)_

### Нормализация deferred-функции через closure

Поле `fn func()` **не имеет** аргументов и return-значений. Как это работает, если вы defer-ите функцию с аргументами?
...
В Go существует **три вида** defer-вызовов: **open-coded defer**, **stack defer** и **heap defer**:
...
_Figure 115. Calling stmt defer (src/cmd/compile/internal/ssagen/ssa.go)_
...
_Figure 116. goDeferStmt (src/cmd/compile/internal/escape/call.go)_

Ключевое условие — `e.loopDepth == 1`. Компилятор отслеживает `loopDepth`, начинающийся с 1 (тело основной функции) и увеличивающийся при входе в цикл.
...
Первый шаг — создание **closure** для deferred-функции. Это closure-объект (называемый `funcval`) содержит function pointer `F`, указывающий на wrapper-функцию, и поле аргумента `X0`, хранящее захваченное значение переменной цикла `i`:
...
Сначала runtime получает текущую goroutine (`getg()`). Затем создаёт новую запись `_defer` (`newdefer()`). Новая запись **связывается** с defer chain goroutine через поле `link`:
...
> **Иллюстрация 197.** Assembly проверяет результат deferproc для panic handling _(Рисунок не загружен)_

Если return-значение **не 0** — функция возвращается из-за panic, и код переходит к `runtime.deferreturn`.

Как это работает, если Go-сигнатура `deferproc` не имеет return-значения? Здесь вступает **специальная функция** `return0()`:
...
_Figure 119. return0 (src/runtime/asm_arm64.s)_
...
> **Иллюстрация 198.** Локальный pool пополняется из global pool _(Рисунок не загружен)_
...
Если и локальный, и global pool пусты — runtime выделяет **свежий** `_defer` с heap (`new(_defer)`). Heap allocation дороже и создаёт давление на GC.
...
> **Иллюстрация 199.** Локальные defer pool пополняют global defer pool _(Рисунок не загружен)_

Как Go предотвращает **бесконечный рост** global pool? Garbage collector. В начале каждого GC-цикла он **отвязывает** весь global list, очищает `link` поля и сбрасывает head pointer в `nil`. Вся партия становится обычным мусором и освобождается в следующей sweep-фазе.
...
_Figure 120. deferreturn (src/runtime/panic.go)_
...
> **Иллюстрация 200.** Stack pointer caller идентифицирует принадлежность defer _(Рисунок не загружен)_
...
Heap-allocated defer имеют **стоимость**: heap allocation и overhead кеш-менеджмента. Но `defer` встроен в Go и используется повсеместно. В высоконагруженных системах с тысячами или миллионами goroutine дополнительная стоимость heap allocation **быстро накапливается**. Для решения этой проблемы, начиная с Go 1.13, команда Go ввела **«stack-allocated defers»** как оптимизацию.

Как создать stack-allocated defer на практике? Если функция имеет хотя бы один heap-allocated defer, то другие простые `defer` в той же функции **не используют** open-coded defer, а становятся **stack-allocated**:
...
_Figure 121. Stack-Allocated Defer_

Если deferred-функция **не принимает** аргументов и не возвращает значений (как `printsmt()`), компилятору не нужно создавать wrapper или closure. Runtime может вызвать `deferproc` напрямую с function pointer.

Assembly для stack-allocated defer:

> **Иллюстрация 201.** deferprocStack обрабатывает простые defer на stack _(Рисунок не загружен)_

Компилятор создаёт запись `_defer` **непосредственно на stack frame** функции вместо allocation на heap. Для заполнения stack pointer, program counter и других полей вызывается `runtime.deferprocStack`:
...
_Figure 122. deferprocStack Function (src/runtime/panic.go)_

После этого процесс работает **так же**, как с heap-allocated defer записями, за исключением того, что stack-allocated defer **не добавляются** в defer pool. Они автоматически очищаются при раскрутке stack.
...
_Figure 123. open-coded defers отключены при heap-allocated result parameters (src/cmd/compile/internal/ssagen/ssa.go)_
...
_Figure 124. walkStmt ODEFER node (src/cmd/compile/internal/walk/stmt.go)_
...
В данном примере компилятор может **hardcode** биты 1, 3 и 7 и вызывать напрямую. В более сложных случаях, когда defer скрыт за branch, компилятор **не может знать** на compile-time, должен ли бит быть установлен, и использует условные инструкции.

---

## Xulosa (Mening xulosalarim)

Bu bob `defer` ning nafaqat **foydalanish pattern**larini, balki **ichki implementatsiyasini** — runtime strukturalari, assembly kodi va performance xususiyatlarini chuqur ochib berdi. Mana asosiy xulosalarim:

**1. Defer — bu Go'ning eng elegant, lekin eng noto'g'ri tushunilgan feature'laridan biri.** Ko'pchilik dasturchilar `defer` ni «funksiya oxirida chaqiriladigan cleanup» deb biladi. Lekin **argumentlar darhol baholanadi**, receiver value **copy** qilinadi, va `defer` **funksiyaga** bog'langan, blokka emas. Bu uchta xususiyat eng ko'p bug manbayi. Pointer receiver yoki closure ishlatish — yagona to'g'ri yechim agar latest value kerak bo'lsa.
...
**4. `_defer` record — linked list elementlari.** Har bir `defer` `_defer` struct yaratadi: `{fn, sp, pc, link, heap}`. Barcha `_defer` lar goroutine'ning `g._defer` field'idan boshlanuvchi **singly linked list**. `link` field keyingi `_defer` ga ko'rsatadi. `sp` field qaysi funksiyaga tegishli ekanligini aniqlaydi — `deferreturn` faqat **joriy frame'ning sp'siga mos** keladigan `_defer` larni execute qiladi.
