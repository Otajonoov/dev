...
Oldingi darslarda MVCC bilan tanishдik: PostgreSQL bir row'ni o'zgartirganda uni **joyida yangilamaydi**, balki row'ning yangi **versiyasini** (tuple'ni) yaratadi. Eski versiya darhal yo'q qilinmaydi — u boshqa transaction'larning snapshot'lari uchun hali ham kerak bo'lishi mumkin.
...
5-darsda **page ichidagi tozalash** (in-page vacuum, HOT bilan bog'liq) bilan tanishgan edик. Uni hozirgi «to'liq» vacuum bilan chalkashtirmaslik kerak:
...
Diqqat qiling: dastlabki ikki pointer `dead` emas, balki **`unused`** holatiga o'tdi (page ichi tozalashda `dead` bo'lardi). Sababi — endi ularга index'dan **hech qanday havola qolmadi**, shuning uchun ular butunlay bo'sh deb hisoblanadi va yangi versiyalar uchun ishlatilishi mumkin:
...
> **Nega bu muhim?** Agar page visibility map'da `all_visible` bo'lsa, keyingi vacuum uni **butunlay o'tkazib yuborishi** mumkin (u yerda tozalanadigan narsa yo'q). Bundan tashqari, **Index Only Scan** ham shu belgига tayanadi — table'ga murojaat qilmasdan javob berish mumkin bo'ladi.
...
Endi asosiy seansda row'ni yana yangilaymiz va tozalaймiz:
...
Hammasi **table skanlash**dan boshlanadi (visibility map hisobga olinadi — `all_visible` belgilangan page'lar o'tkazib yuboriladi). O'lik versiyalarning tid (tuple identifikatori) larи xotiradagi maxsus omborga yoziladi.

> **v17 yangiligi:** ilgari (PostgreSQL 17 gacha) tid'lar oddiy massiv'da saqlanardi. Endi ular **prefiks daraxti** (radix tree) ko'rinishida saqlanadi — bu bir necha barobar ixchamroq. Ombor hajmi `maintenance_work_mem` parametri bilan cheklangan (default **64MB**), lekin xotira kerak bo'lganда ajratiladi.
...
- Yuklama vaqt bilan o'zgarganда qat'iy jadval (masalan, «har kuni soat 3 da») ham to'g'ri kelmaydi.
...
- Worker bazaga ulanib, ikki ro'yxat tuzadi: **tozalash kerak** bo'lган va **analiz kerak** bo'lган table'lar.
...
> **Amaliy maslahat:** default 20% (scale_factor 0.2) katta table'lar uchun **juda ko'p**. Masalan, 100 mln row'li table uchun bu 20 mln o'lik versiya to'planishini kutish degani! Katta table'larда bu qiymatni sezilarli **kamaytirish** kerak (masalan 0.01 yoki table darajasida storage parametri orqali).

**2) Yangi row qo'shilganда** (v13'dan). Faqat qo'shiladigan, hech qachon o'chirilmaydigan/o'zgartirilmaydigan table'da o'lik versiya bo'lmaydi, lekin uni ham tozalash kerak — versiyalarni oldindan **freeze** qilish va visibility map'ni yangilash uchun (7-dars). Formula:
...
O'zgargan row'lar soni pol qiymatдан oshsa:
...
`max_dead_tup = 50`, lekin formula bo'yicha `50 + 0.2 × 1000 = 250` bo'lishi kerak edi. Sabab — hali **statistika yo'q**, `INSERT` o'zi statistikани yangilamaydi:
...
> **v14 nozikligi:** `reltuples = -1` — bu maxsus konstanta. U statistikasi **yo'q** table'ни, statistika yig'ilgan bo'sh table'дан farqlaydi. Hisoblashда manfiy qiymat nolga keltiriladi: `50 + 0.2 × 0 = 50`.
...
Katta table bilan sinash uchun 5 mln row qo'shib, hammasini yangilaymiz va `maintenance_work_mem`'ни ataylab 1MB'ga tushiramiz:
...
Vacuum ishlayotganда boshqa seansdan progress'ni tekshiramiz:
...
- `index_vacuum_count` — index bo'yicha nechinchi o'tish. **1'dan katta bo'lsa** — `maintenance_work_mem` bir o'tishda tozalashga yetmagan (bu holatда VERBOSE oxirida `index scans: 3` ko'rinadi);
- `dead_tuple_bytes` / `num_dead_item_ids` — shu o'tishда yig'ilган o'lik versiyalar hajmi va soni.

> Agar `index_vacuum_count` doim 1'дан katta chiqsa — yoki vacuum'ни **tez-tez** chaqiring (har safar kamroq versiya yig'ilsin), yoki `maintenance_work_mem`'ни **oshiring**.

### Autovacuum'ni kuzatish

`need_vacuum` va `need_analyze` view'lari orqali kutayotган table'larni ko'rish mumkin. Lekin standart usul — autovacuum'ning har bir ishini **server log'iga** yozdirish:
...
- Oddiy VACUUM butun table'ни **va barcha index'larни** qayta ishlaydi; page ichi tozalashдан farqli o'laroq, pointer'larни `unused` holatiga o'tkazadi (index havolalari ketgani uchun).
- Vacuum **visibility map** va **free space map**'ни yangilaydi — bu keyingi vacuum'ларни tezlashtiradi va Index Only Scan'ни mumkin qiladi.
- Qaysi versiya **o'lik** ekanini **database horizon** belgilaydi. Uzoq transaction horizon'ни ushlab tursa, vacuum butun bazada o'lik versiyalarни tozalay olmaydi.
- Vacuum **4 bosqichда** ishlaydi: table scan → index cleanup → table cleanup → (ba'zan) table truncation. Faqat oxirgi bosqich faylни kichraytiradi.
- **ANALYZE** query planner uchun statistika yig'adi; vacuum bilan birga (`VACUUM ANALYZE`) yoki alohida bajariladi.
- **Autovacuum** tozalash/analiz'ни table o'zgarish tezligiga qarab avtomatik ishga tushiradi: `launcher` + `worker`'lar. Ishga tushish `threshold + scale_factor × reltuples` formulasi bilan aniqlanadi.
...
2. VACUUM'дан keyin pointer nega `dead` emas, `unused` holatiga o'tadi? `unused` va `dead` orasidagi farq nima?
3. `all_visible` belgisi qaysi ikki joyда saqlanadi va u qanday amaliy foyda beradi (kamida ikkita)?
4. Uzoq davom etayotgан transaction butun bazada vacuum ishiga qanday ta'sir qiladi? VERBOSE'даги «dead but not yet removable» qatori nimани anglatadi?
5. VACUUM'ning to'rtta bosqichini tartib bilan sanang. Qaysi bosqich table faylини kichraytiradi va qanday shartlar bilan?
6. `VACUUM VERBOSE` natijasida `index scans: 3` ko'rinsa, bu nimani bildiradi va uni qanday tuzatish mumkin?
7. Autovacuum bir table'ни tozalash uchun ishga tushish formulasini yozing. 100 mln row'li table'да default `scale_factor = 0.2` nega muammoli?
8. `autovacuum_max_workers`'ни oshirsangiz, autovacuum tezligini oshirish uchun yana qaysi parametrни o'zgartirish kerak va nega?
