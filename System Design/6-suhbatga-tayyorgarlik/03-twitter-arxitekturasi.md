...
## Go amaliyoti — timeline'ni o'qish (read path)

Fan-out **yozish** (write) — bu 2-Amaliyotdagi `FanoutTweet`. Bu yerda uning juftini — timeline'ni **o'qish** yo'lini yozamiz. Muhim g'oya: `feed:{user}` sorted set faqat **tweet_id**'larni saqlaydi (arzon), tweet **mazmuni** esa alohida keshda. Shuning uchun o'qish ikki qadam.

### 1-qadam: feed'dan tweet_id'larni sahifalab olish

```go
// --- feed:{user} sorted set'dan bir sahifa tweet_id (yangidan eskiga) ---
func GetTimeline(rdb *redis.Client, userID int64, page, size int) ([]int64, error) {
    key := fmt.Sprintf("feed:%d", userID)
    start := int64(page * size)
    stop := start + int64(size) - 1
    // ZRevRange = eng katta score (eng yangi tweet) birinchi
    ids, err := rdb.ZRevRange(context.Background(), key, start, stop).Result()
    if err != nil {
        return nil, err
    }
...
**Notional machine:** sorted set'da score = tweet vaqti; `ZRevRange` teskari (yangidan eskiga) tartibda qaytaradi — saralash kerak emas, u allaqachon tartibli. `start/stop` sahifalash O(log N + size), chunki Redis sorted set — skip-list. Aynan shu tezlik 100 000 read QPS'ni ko'tarishga imkon beradi (2-bosqich hisobi).

### 2-qadam: tweet_id -> tweet mazmuni (hydration, batch)

ID'lar ro'yxati bor; endi ularning matnini olamiz. **N ta ID uchun N ta so'rov yubormaymiz** (N+1 muammosi) — ommaviy (batch) olamiz.

```go
// --- tweet_id'lar -> mazmun: avval keshdan MGET, MISS'larni DB'dan BITTA so'rovda ---
func HydrateTweets(rdb *redis.Client, db *sql.DB, ids []int64) (map[int64]Tweet, error) {
    result := make(map[int64]Tweet)
    keys := make([]string, len(ids))
    for i, id := range ids {
        keys[i] = fmt.Sprintf("tweet:%d", id)
    }
    vals, _ := rdb.MGet(context.Background(), keys...).Result() // ommaviy kesh o'qish

    var missing []int64
    for i, v := range vals {
        if v == nil {
            missing = append(missing, ids[i]) // cache MISS
            continue
        }
        var t Tweet
        json.Unmarshal([]byte(v.(string)), &t)
        result[t.ID] = t
    }
    // MISS bo'lganlarni DB'dan bitta so'rov bilan (ANY = ro'yxat bo'yicha)
    if len(missing) > 0 {
        rows, _ := db.Query("SELECT id, content FROM tweets WHERE id = ANY($1)", pq.Array(missing))
        defer rows.Close()
        for rows.Next() {
            var t Tweet
            rows.Scan(&t.ID, &t.Content)
            result[t.ID] = t
        }
    }
    return result, nil
...
**Notional machine:** bu — **ikki qatlamli** kesh dizayni. `feed:{user}` — kim nimani ko'radi (tartib), `tweet:{id}` — tweet mazmuni. Nega ajratilgan? Chunki bir tweet million odamning feed'ida bo'lishi mumkin, lekin matn **bitta** nusxada `tweet:{id}`da yotadi — xotira tejaladi va tweet o'zgarsa bir joyda yangilanadi. O'qish jami: 1 ta `ZRevRange` + 1 ta `MGET` (+ kamdan-kam DB batch) — p99 < 200ms shundan keladi.

> 🤔 **O'ylab ko'r:** Nega tweet matnini to'g'ridan-to'g'ri `feed:{user}` sorted set'ning o'ziga (member sifatida) saqlamaymiz — shunda hydration ham kerak bo'lmasdi-ku?

<details>
<summary>💡 Javobni ko'rish</summary>

Chunki bitta tweet o'rtacha 200 (mashhurlarda millionlab) feed'ga tushadi. Agar matnni har feed'ga nusxalasak, xotira **200 barobar** ko'payadi va tweet tahrirlansa (yoki o'chirilsa) uni **hamma** feed'da qidirib yangilash kerak. ID'ni saqlab, matnni bitta `tweet:{id}`da tutish — xotirani tejaydi va yangilashni bitta joyga jamlaydi. Bu — normalizatsiya bilan tezlik orasidagi klassik trade-off.
</details>
