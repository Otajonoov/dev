...
4-bosqichda `HandleWS` va `writeLoop`'ni ko'rdik. Endi eng muhim yetishmayotgan qismni yig'amiz: **Hub** (server ichida xabarni to'g'ri client'ga yo'naltiruvchi markaz), presence va Cassandra schema.

### Hub — bir server ichida xabar yo'naltirish

Har Connection Server o'ziga ulangan minglab client'ni bir joyda saqlashi va kelgan xabarni to'g'ri client'ga uzatishi kerak. Buni **Hub** qiladi: `map[userID]*Client` va uchta kanal (`register`, `unregister`, `route`).

```go
// --- 1-qadam: Hub bir serverdagi barcha ulangan clientlarni saqlaydi ---
type Hub struct {
    clients    map[string]*Client // userID -> Client
    register   chan *Client
    unregister chan *Client
    route      chan Message       // yo'naltirilishi kerak bo'lgan xabarlar
}

// --- 2-qadam: BITTA goroutine hammasini ketma-ket boshqaradi (mutex kerak emas) ---
func (h *Hub) Run() {
    for {
        select {
        case c := <-h.register:
        ...
        case msg := <-h.route:
            // --- 3-qadam: qabul qiluvchi SHU serverda ulanganmi? ---
            if to, ok := h.clients[msg.To]; ok {
                data, _ := json.Marshal(msg)
                select {
                case to.Send <- data: // uning "pochta qutisi"ga tashla
                default:              // bufer to'la = sekin/uzilgan client
                    close(to.Send)
                    delete(h.clients, to.ID)
                }
            }
            // shu serverda topilmasa -> Message Router (Kafka/Redis) ga uzatiladi
        }
    }
}
```

**Notional machine:** `clients` map'iga **faqat bitta goroutine** (`Run`) tegadi — shuning uchun mutex/lock umuman kerak emas, race bo'lmaydi. `register`, `unregister`, `route` uchta kanal barcha o'zgarishlarni **navbatga soladi** (seriyalashtiradi). Bu — Go falsafasi: "xotirani baham ko'rish uchun aloqa qil, aloqa uchun xotirani baham ko'rma". `to.Send <- data` xabarni qabul qiluvchining `writeLoop`'iga o'tkazadi, u esa WebSocket orqali telefonga push qiladi. Agar qabul qiluvchi **boshqa** serverda bo'lsa — 4-bosqichdagi Message Router uni to'g'ri serverga yetkazadi.

> 🤔 **O'ylab ko'r:** Nega Hub'da `clients` map'ini himoya qilish uchun mutex ishlatmadik, garchi minglab goroutine (har ulanish uchun) bir vaqtda ishlayotgan bo'lsa ham?

<details>
<summary>💡 Javobni ko'rish</summary>

Chunki map'ga **hech bir** ulanish goroutine'i to'g'ridan-to'g'ri tegmaydi — ular faqat kanalga (`register`/`route`) xabar tashlaydi. Map'ni **yagona** `Run` goroutine'i o'zgartiradi, u ham ketma-ket (select bir vaqtda bitta case). Bir vaqtda ikki goroutine map'ga yozmagani uchun race yo'q, mutex ortiqcha. Bu — mutex o'rniga **kanal bilan sinxronizatsiya** — ko'pincha soddaroq va xatosizroq.
</details>

### Presence — online/offline holat (Go)

5-bosqichda presence g'oyasini (heartbeat + Redis TTL) ko'rdik. Kod ko'rinishi:

```go
type PresenceService struct{ redis *redis.Client }

// --- online: 30 soniyalik TTL bilan yozamiz ---
func (p *PresenceService) SetOnline(userID string) {
    p.redis.Set(context.Background(), "presence:"+userID, "online", 30*time.Second)
}
// --- heartbeat: har 15 soniyada TTL yangilanadi ---
func (p *PresenceService) Heartbeat(userID string) {
    p.redis.Expire(context.Background(), "presence:"+userID, 30*time.Second)
}
// --- tekshirish: kalit hali bor bo'lsa online ---
func (p *PresenceService) IsOnline(userID string) bool {
    return p.redis.Get(context.Background(), "presence:"+userID).Err() == nil
}
```

**Notional machine:** heartbeat (15s) TTL'dan (30s) **ikki barobar tez** — bitta o'tkazib yuborilgan heartbeat foydalanuvchini "offline" qilib qo'ymaydi (bardoshlilik zaxirasi). Telefon uzilsa, heartbeat kelmaydi, 30 soniyada Redis kalitni **o'zi o'chiradi** → `IsOnline` `false` qaytaradi. Alohida "offline qil" xabari umuman kerak emas.

### Cassandra schema — xabar tarixi va offline queue

Xabarlarni (15 TB/kun, 2-bosqich hisobi) saqlash uchun **Cassandra** ideal: yuqori write throughput, time-series ma'lumot, horizontal scaling.

```sql
CREATE TABLE messages (
    conversation_id UUID,        -- partition key: bir suhbat = bir tugunda
    message_id      TIMEUUID,    -- vaqt bilan tartiblangan noyob ID
    sender_id       TEXT,
    text            TEXT,
...
-- So'nggi 50 xabarni olish (bir partition ichida tez range o'qish):
SELECT * FROM messages WHERE conversation_id = ? LIMIT 50;
```

**Notional machine:** `conversation_id` — **partition key**: bitta suhbatning barcha xabarlari bitta tugunda (node) yonma-yon yotadi, shuning uchun "so'nggi 50 xabar" bitta tez o'qish. `message_id` **TIMEUUID** (vaqt ichiga qurilgan) va `CLUSTERING ORDER BY ... DESC` tufayli xabarlar diskda **yangidan eskiga** tartiblangan holda saqlanadi — saralash kerak emas. Offline foydalanuvchining "pending" xabarlari ham shu yerda `status='sent'` bo'lib kutadi; u ulangach shu jadvaldan o'qib yetkaziladi.
...
- **Hub** — server ichida `map[userID]*Client` va kanallar orqali xabarni yo'naltiradi (mutexsiz, bitta goroutine).
- Presence — Redis TTL + heartbeat; delivery — offline queue + ack; xabar tarixi — Cassandra (partition = conversation).
